...
test_size = (1 - train_size) / 2        # test_size == validation_size

num_testHam = int(np.floor(num_allHam * test_size))
num_validHam = num_testHam
num_trainHam = num_allHam - 2 * num_testHam
num_testSpam = int(np.floor(num_allSpam * test_size))
num_validSpam = num_testSpam
num_trainSpam = num_allSpam - 2 * num_testSpam
...
for threshold in np.arange(0.05, 1, 0.001):
    ...
    for ham in validHam_index:
        result = sum([math.log(token_likelihood_ham[token]) - math.log(token_likelihood_spam[token])
                     for token in ham]) + math.log(p_ham) - math.log(p_spam)     # result = log(1/p-1)
    ...
    for spam in validSpam_index:
        result = sum([math.log(token_likelihood_ham[token]) - math.log(token_likelihood_spam[token])
                     for token in spam]) + math.log(p_ham) - math.log(p_spam)     # result = log(1/p-1)
        if result < math.log(1/threshold-1):
            num_correct += 1
            num_correct_spam += 1
...
print(
    f'Best accuracy: {best_accuracy}     Threshold: {best_smallest_threshold} - {best_largest_threshold}')
print(
    f'Best accuracy HAM: {best_accuracy_ham}     Best accuracy SPAM: {best_accuracy_spam}')
